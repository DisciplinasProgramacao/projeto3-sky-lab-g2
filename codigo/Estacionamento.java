...
public class Estacionamento implements IDataToText {
	...
	/**
 * Gera um relatório da arrecadação total de todos os estacionamentos, ordenado de forma decrescente.
 *
 * @return Uma String contendo o relatório da arrecadação.
 */
public String relatorioArrecadacaoDecrescente() {
    StringBuilder relatorio = new StringBuilder();
    relatorio.append("Relatório de Arrecadação Total dos Estacionamentos:\n");

    // Calcula a arrecadação total de cada estacionamento e os ordena de forma decrescente
    List<Map.Entry<Estacionamento, Double>> arrecadacaoPorEstacionamento = clientes.stream()
            .flatMap(cliente -> cliente.getVeiculos().stream())
            .collect(Collectors.groupingBy(Veiculo::getEstacionamento, Collectors.summingDouble(Veiculo::totalArrecadado)))
            .entrySet().stream()
            .sorted(Map.Entry.<Estacionamento, Double>comparingByValue().reversed())
            .collect(Collectors.toList());

    // Formata o relatório
    for (Map.Entry<Estacionamento, Double> entry : arrecadacaoPorEstacionamento) {
        Estacionamento estacionamento = entry.getKey();
        Double totalArrecadado = entry.getValue();
        relatorio.append(String.format("Estacionamento: %s - Total Arrecadado: R$ %.2f\n", estacionamento.getNome(), totalArrecadado));
    }
